...
out_dir = '/data4/cotar/'
gaia_data = Table.read(out_dir+'Gaia_dr2_orbital_derivatives.fits')#[:4000000]
clusters = Table.read(out_dir+'Gaia_open_clusters_analysis_October-GALAH-clusters/Cluster_members_Gaia_DR2_Kharchenko_2013_init.fits')
...
for i_c in range(data_transform.shape[1]):
    ...
    # plt.hist(data_transform[:, i_c], bins=75, range=np.nanpercentile(data_transform[:, i_c], [1, 99]))
    # plt.axvline(np.nanpercentile(data_transform[:, i_c], 2), lw=1, ls='--', c='black')
    # plt.axvline(np.nanpercentile(data_transform[:, i_c], 98), lw=1, ls='--', c='black')
    # plt.axvline(np.nanmedian(data_transform[:, i_c]), lw=1, ls='--', c='blue')
    # plt.axvline(np.nanmedian(data_transform[:, i_c]) + np.nanstd(data_transform[:, i_c]), lw=1, ls='--', c='blue')
    # plt.axvline(np.nanmedian(data_transform[:, i_c]) - np.nanstd(data_transform[:, i_c]), lw=1, ls='--', c='blue')
    # plt.savefig('orbit_train_set_' + use_tsne_cols[i_c] + '.png', dpi=200)
    # plt.close()
    # standardization to std=1, mean=0
    data_transform[:, i_c] = (data_transform[:, i_c] - np.nanmedian(data_transform[:, i_c])) / np.nanstd(data_transform[:, i_c])
    # normalization to range -1 ... 1
    # c_lim = np.nanpercentile(data_transform[:, i_c], [2, 98])
    # data_transform[:, i_c] = (data_transform[:, i_c] - c_lim[0]) / (c_lim[1] - c_lim[0]) * 2. - 1.
...
for i_r in range(len(ra_rand)):
    ...
    #             TSNE
    # -----------------------------
    suffix = suffix_coord+'_std_p30_t04_all'
    fits_out = 'gaia_tsne_embeded'+suffix+'.fits'
    if not isfile(fits_out):
        tsne_class = TSNE_multi(n_components=2, perplexity=30, n_iter=1000, n_iter_without_progress=350, init='random',
                                verbose=1, method='barnes_hut', angle=0.4, n_jobs=70)
